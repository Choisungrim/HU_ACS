...
public interface ZoneMasterRepository extends JpaRepository<ZoneMaster, ZoneMasterId> {
    ...
    List<ZoneMaster> findByMapUuid(@Param("mapUuid")Long mapUuid);

    // site_cd 기준으로 영역 조회
    @Query("SELECT z FROM ZoneMaster z WHERE z.site_cd = :siteCd AND z.usable_fl = 1")
    List<ZoneMaster> findUsableZonesBySite(@Param("siteCd")String siteCd);
    ...
    ZoneMaster findByKey(@Param("zoneId")String zoneId,@Param("mapUuid") Long mapUuid, @Param("siteCd") String siteCd);
}
